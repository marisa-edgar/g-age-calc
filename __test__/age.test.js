...
describe('User',()=>{
  ...
  test('should correctly return age in mercury years', ()=>{
    ...
    expect(user1.name).toEqual("Risa");
    expect(user1.age).toEqual(32);
    expect(user1.mercuryAge()).toEqual(133.33333333333334);
  });
  test('should correctly return age in venus years', ()=>{
    const user1 = new User("Risa",32);
    expect(user1.name).toEqual("Risa");
    expect(user1.age).toEqual(32);
    expect(user1.venusAge()).toEqual(51.612903225806456);
  });
  test('should correctly return age in mars years', ()=>{
    const user1 = new User("Risa",32);
    expect(user1.name).toEqual("Risa");
    expect(user1.age).toEqual(32);
    expect(user1.marsAge()).toEqual(17.02127659574468);
  });
  test('should correctly return age in jupiter years', ()=>{
    const user1 = new User("Risa",32);
    expect(user1.name).toEqual("Risa");
    expect(user1.age).toEqual(32);
    expect(user1.jupiterAge()).toEqual(2.69814502529511);
  });
  test('should correctly return how many years past life expectancy the user has lived', ()=>{
  const user2 = new User("Ester",80);
  expect(user2.yearsPastExpectancy()).toEqual(8);
  });
  test('should correctly return the years the user has left until life expectancy is reached', ()=>{
    const user1 = new User("Risa",32);
    expect(user1.yearsPastExpectancy()).toEqual(40);
  });
});